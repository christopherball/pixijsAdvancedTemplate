...
vec3 calclight(vec3 p, vec3 rd) {
    ...
    vec3 n = normalize(vec3(dist(p + eps.yxx).x - dist(p - eps.yxx).x, dist(p + eps.xyx).x - dist(p - eps.xyx).x, dist(p + eps.xxy).x - dist(p - eps.xxy).x));
...
void main() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (d.x < 0.001) {
            vec3 bc = d.y < 0.5 ? vec3(1.0, .8, 0.) : vec3(0.8, 0.0, 1.0);
